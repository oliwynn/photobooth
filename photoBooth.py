...
def main():
    ...
    while True:
        if GPIO.input(23):
             ...
             os.system("raspistill -o SelfieStation/image" + str(ts)+".jpg -t 1 -n -h 612 -w 612")
             os.system("aplay camera.wav")
             createThumb("image" + str(ts)+".jpg")
             print "photo taken...."
             print "reset....."
...
def createThumb(imageInput):
    size = 200, 200
    im = Image.open("SelfieStation/"+imageInput)
    im.thumbnail(size)
    im.save("SelfieStationThumb/" + imageInput, "JPEG")
